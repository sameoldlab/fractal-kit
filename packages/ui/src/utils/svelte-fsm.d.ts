...
type BaseStates<State extends BaseState = BaseState> = Record<
	State,
	BaseActions
>
...
type DetectFallBackState<State extends BaseState> = State extends '*'
	? string
	: State
...
type ExtractObjectValues<O extends object> = O[keyof O]

type GetActionFunctionMapping<Actions extends BaseActions> = {
	[Key in Exclude<
		keyof Actions,
		'_enter' | '_exit'
	>]: Actions[Key] extends BaseState
...
type Subscribe<S extends BaseState> = (
...
declare const svelteFsm: <
	Sts extends Readonly<BaseStates>,
	S extends ExtractStates<Sts>
>(
	state: S,
	states: Sts
